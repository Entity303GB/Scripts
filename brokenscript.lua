local DevMode = true -- Set to true to enable Developer Mode (Bypass Key System)
...
--[[
Sync v2.7 - Fixed Version (Race Conditions Patched)
]]

-- [[ FIX: Global Animation Mutex ]]
local UIActionLock = false
...
if DevMode or (status and status.s == 1) then
    ...
    local function showPopup(cmd)
        if UIActionLock then return end
        currentPopupCmd = cmd
        popupinput.Text = ""
   ...
   rejoinbtn.MouseButton1Click:Connect(function()
    ...
    anim(label, 0.3, {
        ...
        -- [[ FIX: Main Navigation Logic with Mutex ]]
        iconbtn.MouseButton1Click:Connect(function()
            ...
            if minimized then
                ...
                hasDragged = false
                UIActionLock = false -- Release lock
                return
            end
    ...
    while true do
        task.wait(1)
        -- Added check for UIActionLock to prevent minimizing during interactions
        if not minimized and not UIActionLock and (tick() - lastinteract) >= Config.MinimizeDelay and not tabopen then
            minimized = true
            UIActionLock = true -- Lock while minimizing
    ...
    if not IsMobile then
        ...
        local CmdBarLock = false -- Lock for command bar animation

        UserInputService.InputBegan:Connect(function(input, gpe)
            if gpe then return end
            
            if input.KeyCode == Config.CommandBarKey then
                if CmdBarLock then return end
                CmdBarLock = true
